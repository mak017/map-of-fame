...
import { followingState, profileState } from "../../js/store";
import { getFollowing, unfollowUser } from "../../js/api/follow";
...
const fetchUsers = async (offset = 0, isNewFetch) => {
  followingState.setIsLoading(isNewFetch);
  followingState.setIsShowSpinner(true);
  const response = await getFollowing(token, MAX_ITEMS_PER_PAGE, offset);
  const { success, result } = response;
  if (success && result) {
    if (isNewFetch) {
      followingState.setList([]);
    }
    newBatch = result.following ? [...result.following] : [];
    followingState.setList([...$followingState.list, ...newBatch]);
    followingState.setHasMore(newBatch.length === MAX_ITEMS_PER_PAGE);
    followingState.setTotal(result.followingCount);
    followingState.setIsFetched(true);
  }
  followingState.setIsLoading(false);
  followingState.setIsShowSpinner(false);
};

onMount(() => {
  parentModal = document.getElementById("following-modal");

  if ($followingState.scrollOffset) {
    setTimeout(() => {
      parentModal.scrollTo({
        top: $followingState.scrollOffset - parentModal.offsetHeight / 2,
      });
      followingState.setScrollOffset(0);
    }, 0);
  }
});

$: if (!$followingState.isFetched) {
  fetchUsers(0, true);
}

const navigateToUserProfile = (user) => {
  const element = document.querySelector(`[data-scroll-element="${user.id}"]`);
  followingState.setScrollOffset(element.offsetTop);
  profileState.reset();
  $goto("/@:username", { username: user.username });
};

const handleLoadMore = () => {
  if ($followingState.isShowSpinner) {
    return;
  }

  const offset = $followingState.offset + MAX_ITEMS_PER_PAGE;
  followingState.setOffset(offset);
  fetchUsers(offset);
};
...
const handleUnfollowBtnClick = (user) => async () => {
  const { success } = await unfollowUser(token, user.id);
...
<div class="container">
  <div class="data">
    <div class="users">
      {#if !!$followingState.list.length || $followingState.isShowSpinner}
        {#if !$followingState.isLoading}
          ...
          {/if}
          {#each $followingState.list as user (user.id)}
            <a
              ...
              <div class="cell followers">
                ...
                <div class="value">
                  ...
                  <button
                    ...
                    on:click={handleUnfollowBtnClick(user)}>Unfollow</button>
                </div>
              </div>
              {#if isMobile()}
                <div class="overlay">
        ...
        {/if}
      {:else if !$followingState.isShowSpinner}
